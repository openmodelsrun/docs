...
| `sort` | string | No | `recency` | Sort order: `name` (A-Z), `recency` (newest first), `context_window` (largest first), `providers` (most first) |
...
Returns models ranked by a relevance score combining recency and provider availability. Useful for homepage "Popular Models" sections.

### Query Parameters

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `limit` | integer | No | `12` | Number of models to return (8–12 recommended) |
...
```
score = 0.7 × recency_factor + 0.3 × provider_factor
```

- **recency_factor**: 1.0 for models created within 6 months, linear decay to 0.0 at 24 months
- **provider_factor**: model's mapping count / max mapping count across all models

Models with `updated_at` older than 24 months AND zero provider mappings are excluded.

### Example Request

```bash
curl "https://api.openmodels.run/api/models/popular?limit=8"
```
...
[
  {
    "id": "deepseek-v4",
    "name": "DeepSeek V4",
    "description": "DeepSeek's fourth-generation model with enhanced reasoning.",
...
Returns an array of model objects (same schema as individual model responses), sorted by relevance score descending.

### Caching

Results are cached with a 5-minute TTL. New models appear in the ranking within 1 hour of ingestion.
